...
extension GradientDesignable where Self: UIView {
  ...
  public func configureGradient() {
    ...
    public func configureGradientButton(button : UIButton) {
        configureGradientButton(in: button)
    }
...
extension GradientDesignable {
  ...
  func configureGradient(in view: UIView) {
    ...
    func configureGradientButton(in button: UIButton) {
        button.subviews.filter { $0 is PrivateGradientView }.forEach {
            $0.removeFromSuperview()
        }
...
//        let gradientView = PrivateGradientView(frame: button.bounds, gradientLayer: gradientLayer)
        gradientLayer.frame = button.bounds
